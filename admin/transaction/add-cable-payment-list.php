		    ...
		    $_SESSION['add_user_error_msg'] = "<div class='alert alert-danger'><button class='close' data-dismiss='alert'>&times;</button>Please Fill all the Fileds!</div>";
                   ...
                   $stmt = $reg_user->runQuery("UPDATE user_has_cable SET due_date='$due_date' WHERE user_id='$user_id' AND due_date<'$due_date'");
            		...
            		$_SESSION['add_user_error_msg'] = "<div class='alert alert-success'><button class='close' data-dismiss='alert'>&times;</button><strong>New Cable TV Payment!</strong> Added Successfully.</div>";
                		header("Location: /add-cable-payment-list");
                		exit();
		}
...
<div class="site-content" ng-app="WVCApp" ng-controller="WVCCtrl">
				<div class="content-area py-1">
					<div class="container-fluid">
						<h4>Add New Cable Payment</h4>
						<ol class="breadcrumb no-bg mb-1">
							<li class="breadcrumb-item"><a href="/cable-transaction-list">Cable Payment List</a></li>
							<li class="breadcrumb-item active"><a href="#">Add New Cable Payment</a></li>
						</ol>
						<div class="box box-block bg-white">
						    <div class="error-div"><?php if(isset($_SESSION['add_user_error_msg'])){ echo $_SESSION['add_user_error_msg']; unset($_SESSION['add_user_error_msg']); } ?></div>

							<form class="form-horizontal" action="/add-cable-payment-list" method="post" name="edit_ad" enctype="multipart/form-data" ng-submit="submit()">
								<div class="row">
									<div class="col-md-6">
									<div class="form-group">
										...
										<?php
										if(isset($_GET['id'])){
                                        $stmt = $reg_user->runQuery("SELECT *,us.id as UserID FROM user_has_cable uc INNER JOIN user us ON us.id=uc.user_id INNER JOIN user_as_smartcard uas ON us.id=uas.user_id WHERE us.id='$user_id' ORDER BY uas.smart_card_no ASC");
                                        }else{
                                        $stmt = $reg_user->runQuery("SELECT *,us.id as UserID FROM user_has_cable uc INNER JOIN user us ON us.id=uc.user_id INNER JOIN user_as_smartcard uas ON us.id=uas.user_id ORDER BY uas.smart_card_no ASC");
                                        }
                                				
                                				$stmt->execute();
                                				for($i=0; $stmt1 = $stmt->fetchObject(); $i++){ ?>
                                				<option <?php if(isset($_POST['user_id']) && $_POST['user_id']==$stmt1->UserID){ echo "selected"; }else{ echo ""; } ?> value="<?php echo $stmt1->UserID; ?>"><?php echo $stmt1->smart_card_no; ?></option>
                                	    <?php } ?>
                                			</select>
										</div>
									</div>
									<div class="form-group">
										...
										if(isset($_GET['id'])){
                                        	$stmt = $reg_user->runQuery("SELECT *,cp.id as PackageID FROM cable_package cp INNER JOIN user_has_cable uc ON uc.package_id=cp.id WHERE uc.user_id='$user_id'");
                                        }else{
                                        	$stmt = $reg_user->runQuery("SELECT *,cp.id as PackageID FROM cable_package cp");
                                        }
                                				$stmt->execute();
                                				for($i=0; $stmt1 = $stmt->fetchObject(); $i++){ ?>
                                				<option <?php if(isset($_POST['package_id']) && $_POST['package_id']==$stmt1->PackageID){ echo "selected"; }else{ echo ""; } ?> value="<?php echo $stmt1->PackageID; ?>"><?php echo $stmt1->name; ?></option>
                                	    <?php } ?>
                                			</select>
									</div>
									<div class="form-group">
									...
									<div class="form-group">
										...
										<input type="text" placeholder="BB NO" name="bb_num" value="<?php if(isset($_POST['bb_num'])){ echo $_POST['bb_num']; } ?>" class="form-control" >
									</div>
									<div class="form-group">
									    <label><b>Extra</b></label>
										<input type="text" placeholder="Extra" name="extra" value="<?php if(isset($_POST['extra'])){ echo $_POST['extra']; } ?>" class="form-control" >
									</div>
								</div>
								<div class="col-md-6">
									<div class="form-group">
									...
									<input type="text" id="sandbox-date-container1" placeholder="Month / Date" name="month_date" value="<?php if(isset($_POST['month_date'])){ echo $_POST['month_date']; } ?>" class="form-control" >
									</div>
									<div class="form-group">
									    <label><b>Monthly Amount (Optional)</b></label>
										<input type="text" class="form-control" placeholder="Amount" name="month_amount" value="<?php if(isset($_POST['month_amount'])){ echo $_POST['month_amount']; } ?>" >
									</div>
