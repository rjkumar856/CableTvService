...
if(isset($_POST['cus_update_user'])){
		...
		if(empty($user_id) || empty($full_name) || empty($email) || empty($phone) || empty($country) || empty($city) || empty($state) || empty($address_1) || empty($user_type)){
		    ...
		    if(in_array(1,$user_type) && empty($broadband_package)){
    		 ...
    		 if(in_array(2,$user_type) && empty($cable_package)){
    		      $_SESSION['add_user_error_msg'] = "<div class='alert alert-danger'><button class='close' data-dismiss='alert'>&times;</button>Please Select Cable Package!</div>";
    			    ...
    			    $stmt = $reg_user->runQuery("UPDATE user SET userid='$user_id',full_name='$full_name',email='$email',phone='$phone',user_type='$user_type[0]',card_no='$card_no',status='$user_status' WHERE id='$UserID'");
                    $stmt->execute();
                    
                    $stmt_addr = $reg_user->runQuery("UPDATE profile SET address_1='$address_1',zip_code='$pincode',city_id='$city',state_id='$state',country_id='$country' WHERE user_id='$UserID'");
                    $stmt_addr->execute();
                    ...
                    if(in_array(1,$user_type)){
                            $stmt_addr = $reg_user->runQuery("UPDATE user_has_broadband SET package_id='$broadband_package',due_date='$broadband_due' WHERE user_id='$UserID'");
                            $stmt_addr->execute();
                    }
            		...
            		header("Location: /edit-user-list?id=".$id);
            		exit();
		 }
		 }
		}
							...
							<form class="form-horizontal" method="post" name="edit_ad" enctype="multipart/form-data">
								...
								<div class="col-md-6">
									...
									<div class="form-group">
									    ...
									    <input type="checkbox" name="network_type[]" <?php if($row_user['UBActive']){ echo "checked"; } ?> id="network_type1" class="form-control" value="1"> 
									    <label for="network_type1" class="label-for-channel"><span><i class="fa fa-check" aria-hidden="true"></i></span> Broadband</label>
									    
									    <input type="checkbox" name="network_type[]" <?php if($row_user['UCActive']){ echo "checked"; } ?> id="network_type2" class="form-control" value="2"> 
									...
									<div class="form-group">
										...
										<?php
                                				...
                                				$stmt->execute();
                                				for($i=0; $stmt1 = $stmt->fetchObject(); $i++){ ?>
                                				<option <?php if($row_user['UBpackage_id'] == $stmt1->PackID){ echo "selected"; }else{ echo ""; } ?> value="<?php echo $stmt1->PackID; ?>"><?php echo $stmt1->NetName.'-'.(($stmt1->PackName)?$stmt1->PackName:$stmt1->Speed."-".$stmt1->Data_Transfer." @Rs.".$stmt1->Traiff); ?></option>
                                	    <?php } ?>
                                			</select>
									</div>
									<div class="form-group">
									    <label><b>Broadband Due Date</b></label>
										<input type="text" id="Broadband-date-container" placeholder="Due Date" name="broadband_due" value="<?php echo $row_user['UBdue_date']; ?>" class="form-control">
									</div>
									<div class="form-group">
