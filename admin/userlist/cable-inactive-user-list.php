				...
				<div class="content-area py-1">
					<div class="container-fluid">
						...
						<div class="box box-block bg-white">
                            ...
                            <?php
                            $stmt = $reg_user->runQuery("SELECT *, us.id as UsrID FROM user us INNER JOIN user_has_cable ub ON ub.user_id=us.id INNER JOIN cable_package cp ON cp.id=ub.package_id WHERE date(ub.due_date)<CURDATE()");
                            $stmt->execute();
                            for($i=0; $stmt1 = $stmt->fetchObject(); $i++)
                            {
                            ?><tr>	
                            <td class="jsgrid-cell jsgrid-control-field jsgrid-align-center" style="width: 50px;">
										...
										<td>
									<?php $stmt13 = $reg_user->runQuery("SELECT * FROM user_as_smartcard WHERE user_id = '".$stmt1->UsrID."' ");
$stmt13->execute();
$row13 = $stmt13->fetch(PDO::FETCH_ASSOC); 
echo $row13['smart_card_no']; ?></td>
