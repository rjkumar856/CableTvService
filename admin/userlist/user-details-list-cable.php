...
<?php
$stmt = $reg_user->runQuery("SELECT *, us.id as UsrID FROM user us INNER JOIN user_has_cable ub ON ub.user_id=us.id LEFT JOIN user_as_smartcard uas ON uas.user_id=us.id INNER JOIN cable_package cp ON cp.id=ub.package_id");
$stmt->execute();
for($i=0; $stmt1 = $stmt->fetchObject(); $i++)
{
?><tr>	
