...
if(isset($_POST['cus_update_user'])){
		...
		if(empty($user_id) || empty($full_name) || empty($email) || empty($phone) || empty($broadband_package) || empty($city) || empty($broadband_due) || empty($address_1) || empty($user_type)){
		    $_SESSION['add_user_error_msg'] = "<div class='alert alert-danger'><button class='close' data-dismiss='alert'>&times;</button>Please Fill all the Fileds!</div>";
		}else{
    			    $stmt = $reg_user->runQuery("UPDATE user SET userid='$user_id',full_name='$full_name',email='$email',phone='$phone',user_type='$user_type',status='$user_status' WHERE id='$UserID'");
                    $stmt->execute();
            		...
            		header("Location: /edit-broadband-user-details?id=".$UserID);
            		exit();
		}
								...
								<div class="col-md-6">
									...
									<div class="form-group">
										...
										<?php
                                				...
                                				$stmt->execute();
                                				for($i=0; $stmt1 = $stmt->fetchObject(); $i++){ ?>
                                				<option <?php if($row_user['user_type']==$stmt1->id){ echo "selected"; }else{ echo ""; } ?> value="<?php echo $stmt1->id; ?>"><?php echo $stmt1->name; ?></option>
                                	    <?php } ?>
                                			</select>
									</div>
									<div class="form-group">
									...
									<div class="form-group">
										...
										<input type="text" id="Broadband-date-container" placeholder="Due Date" name="broadband_due" value="<?php echo $row_user['UBdue_date']; ?>" class="form-control" required>
									</div>
									<div class="form-group">
