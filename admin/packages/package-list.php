...
<?php
$stmt = $reg_user->runQuery("SELECT * FROM channel_package");
$stmt->execute();
for($i=0; $stmt1 = $stmt->fetchObject(); $i++)
{
?><tr>	
<td class="jsgrid-cell jsgrid-control-field jsgrid-align-center" style="width: 50px;">
										<a href="edit-channel-packages?id=<?php echo $stmt1->id;?>"><button class="jsgrid-button jsgrid-edit-button" type="button" title="Edit"></button></a>
										...
										<td><?php echo $stmt1->cp_id;?>
										<?php
										$stmt2 = $reg_user->runQuery("SELECT * FROM cable_package WHERE id = '".$stmt1->cp_id."' ");
$stmt2->execute();
$row = $stmt2->fetch(PDO::FETCH_ASSOC);
echo '('.$row['name'].')'; ?>
										</td>
										<td><?php echo $stmt1->channel_id;?>
										<?php
										$stmt2 = $reg_user->runQuery("SELECT * FROM channel WHERE id = '".$stmt1->channel_id."' ");
$stmt2->execute();
$row = $stmt2->fetch(PDO::FETCH_ASSOC);
echo '('.$row['channel_name'].')'; ?></td>
