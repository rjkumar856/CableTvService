...
if(isset($_POST['cus_add_internet_pack'])){
		...
		if(empty($speed) || empty($after_fub) || empty($gst) || empty($validity) || empty($netword_type) || empty($data_transfer) || empty($traiff) || empty($total)){
		    $_SESSION['add_user_error_msg'] = "<div class='alert alert-danger'><button class='close' data-dismiss='alert'>&times;</button>Please Fill all the Fileds!</div>";
    			    ...
    			    $stmt = $reg_user->runQuery("INSERT INTO internet_pack(Network_id,Package_type_id,Name,Speed,Data_Transfer,After_Fup,Traiff,GST,Total,Validity,serial_number,status) 
    			    VALUES('$netword_type','1','$name','$speed','$data_transfer','$after_fub','$traiff','$gst','$total','$validity','$serial_number','1')");
                    $stmt->execute();
								...
								<div class="row">
									...
									<div class="form-group">
										...
										<div class="input-group">
											<input type="text" class="form-control" placeholder="Serial Number" name="serial_number" value="<?php if(isset($_POST['serial_number'])){ echo $_POST['serial_number']; } ?>" required>
										</div>
									</div>
									<div class="form-group">
