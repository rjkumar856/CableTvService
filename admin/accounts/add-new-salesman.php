...
if(isset($_POST['cus_add_new_user'])){
		...
		if(empty($user_id) || empty($password) || empty($full_name) || empty($email) || empty($phone) || empty($country) || empty($city) || empty($state) || empty($address_1)){
		    $_SESSION['add_user_error_msg'] = "<div class='alert alert-danger'><button class='close' data-dismiss='alert'>&times;</button>Please Fill all the Fileds!</div>";
		}else{
		  
    		    $stmt = $reg_user->runQuery("SELECT * FROM tbl_admin_customers WHERE user_id='$user_id'");
    			$stmt->execute();
    			if($stmt->rowCount()){
    			    ...
    			    $stmt = $reg_user->runQuery("INSERT INTO tbl_admin_customers(user_id,name,lname,password,email,mobile,dob,address,city,state,country,pincode,role,status) 
    			                                                         VALUES('$user_id','$full_name','','$password','$email','$phone','','$address_1','$city','$state','$country','$pincode','2','1')");
                    $stmt->execute();
                    $new_user_id = $reg_user->lasdID();
                    
                    $stmt_addr = $reg_user->runQuery("INSERT INTO sales_location_track(sales_id,latitude,longitude,ip) 
                    VALUES('$new_user_id','','','')");
                    $stmt_addr->execute();
...
<div class="site-content">
				<div class="content-area py-1">
					<div class="container-fluid">
						<h4>Create New Sales User</h4>
						<ol class="breadcrumb no-bg mb-1">
							<li class="breadcrumb-item"><a href="/view-sales-list">Sales User List</a></li>
							<li class="breadcrumb-item active"><a href="#">Add Sales User</a></li>
						</ol>
								...
								<div class="row">
									...
									<div class="form-group">
										...
										<div class="input-group">
											<input type="text" class="form-control" placeholder="Name" name="user_id" value="<?php if(isset($_POST['user_id'])){ echo $_POST['user_id']; } ?>" required>
										</div>
									</div>
									<div class="form-group">
