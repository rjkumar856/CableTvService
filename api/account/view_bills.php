...
if($_SERVER['REQUEST_METHOD']=="POST"){
    if(!empty($_REQUEST)){
		    ...
		    $json = array("response" => 200, "status" => "succes", "message"=>"Data Not Available","items"=>'');
				$stmt = $user->runQuery("SELECT *,wp.id as PaymentID,cp.name as cp_name FROM wvc_payment wp INNER JOIN wvc_transaction wt ON wt.id=wp.transaction_id LEFT JOIN cable_package cp ON cp.id=wp.Package_id LEFT JOIN internet_pack ip ON ip.id=wp.Package_id WHERE wp.user_id='$username' ORDER BY wp.Date_added DESC");
				$stmt->execute();
