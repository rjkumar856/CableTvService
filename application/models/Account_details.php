...
class Account_details extends CI_Model{
    ...
    public function get_user_details($data){
        $sql = "SELECT *, us.id AS customer_id, ci.name AS city_name,st.name AS state_name, co.name AS country_name FROM user us LEFT JOIN profile pr ON us.id=pr.user_id LEFT JOIN city ci ON ci.id=pr.city_id 
        LEFT JOIN state st ON st.id=pr.state_id LEFT JOIN country co ON co.id=pr.country_id WHERE us.id='$data'";        
        $query = $this->db->query($sql);
        return $query->result_array();
    }
    
    public function get_smartcard_details($data){
        $sql = "SELECT * FROM user_as_smartcard WHERE user_id='$data'";        
        $query = $this->db->query($sql);
        return $query->result_array();
    }
    ...
    public function get_user_active_internet_package($data){
        $sql = "SELECT *, ip.id as packageId, ip.Name as Package_name, nt.Name as Provider_name FROM user_has_broadband ub INNER JOIN internet_pack ip ON ip.id=ub.package_id INNER JOIN network_type nt ON nt.id=ip.Network_id WHERE ub.user_id='$data'";
        $query = $this->db->query($sql);
        return $query->result_array();
    }
    ...
    public function insert_payment($data){
        return $this->db->insert('wvc_payment', $data);
    }
